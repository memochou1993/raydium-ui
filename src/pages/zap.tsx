...
import txZap from '@/application/zap/txZap'
import { useZap } from '@/application/zap/useZap'
import { useZapAmountCalculator } from '@/application/zap/useZapAmountCalculator'
import useZapInitCoinFiller from '@/application/zap/useZapInitCoinFiller'
import useZapUrlParser from '@/application/zap/useZapUrlParser'
...
import { useZapTwoElements } from '../hooks/useZapTwoElements'
import { Badge } from '@/components/Badge'
import txUnwrapAllWSOL, { txUnwrapWSOL } from '@/application/zap/txUnwrapWSOL'
...
function ZapEffect() {
  ...
  useZapAmountCalculator()
  return null
}
const { ContextProvider: ZapUIContextProvider, useStore: useZapContextStore } = createContextStore({
...
export default function Zap() {
  return (
    <ZapUIContextProvider>
      <ZapEffect />
      <PageLayout mobileBarTitle="Zap" metaTitle="Zap - Raydium">
        <ZapHead />
        <ZapCard />
        {/* <UnwrapWSOL /> */}
        <KLineChart />
  ...
  const popConfirm = () => {
    ...
    useNotification.getState().popConfirm({
      ...
      onCancel: () => {
        setHasUserTemporaryConfirmed(false)
        setIsConfirmPanelOn(false)
        useZap.setState(directionReversed ? { coin1: undefined } : { coin2: undefined })
      }
    })
  }
...
function ZapCard() {
  ...
  const refreshZap = useZap((s) => s.refreshZap)
  ...
  const zapElementBox1 = useRef<HTMLDivElement>(null)
  const zapElementBox2 = useRef<HTMLDivElement>(null)
  const [hasUISwrapped, { toggleZap: toggleUIZap }] = useZapTwoElements(zapElementBox1, zapElementBox2, {
    ...
    >
      ...
      <div className="mt-5 space-y-5">
        <CoinInputBox
          ...
          onUserInput={(value) => {
            useZap.setState({ focusSide: 'coin1', coin1Amount: value })
          }}
        />
        ...
        <CoinInputBox
          ...
          onEnter={(input) => {
            ...
            if (coin1 && coin1Amount) zapButtonComponentRef.current?.click?.()
          }}
          token={coin2}
      ...
      {isSolToWsol(upCoin, downCoin) || isWsolToSol(upCoin, downCoin) ? (
        <Button
          ...
          componentRef={zapButtonComponentRef}
          validators={[
            {
        ...
        <Button
          ...
          validators={[
            ...
            {
              should: zapable === true || routes?.length === 0,
...
function areTokenPairZapable(targetToken: SplToken | undefined, candidateToken: SplToken | undefined): boolean {
  return (
        ...
        <Row className="mt-5 bg-[#141041] rounded-xl py-2 px-6 mobile:px-4 items-center justify-between">
          <Row className="text-sm font-medium text-[#ABC4FF] items-center ">
            ...
            <Tooltip placement="bottom-right">
              ...
              <Tooltip.Panel>
                <p className="w-80">Price has changed since your zap amount was entered.</p>
              </Tooltip.Panel>
            </Tooltip>
          </Row>
          ...
          <Tooltip placement="bottom-right">
            ...
            <Tooltip.Panel>
              <p className="w-80">
                ...
                avoid failed transactions. This zap will leave you with less than {SOL_BASE_BALANCE} SOL.
              </p>
            </Tooltip.Panel>
          </Tooltip>
        </Row>
...
function ZapCardPriceIndicator({ className }: { className?: string }) {
...
function ZapCardInfo({ className }: { className?: string }) {
  const priceImpact = useZap((s) => s.priceImpact)
  const coin1 = useZap((s) => s.coin1)
  const coin2 = useZap((s) => s.coin2)
  const directionReversed = useZap((s) => s.directionReversed)
  ...
  const focusSide = useZap((s) => s.focusSide)
  const minReceived = useZap((s) => s.minReceived)
  const fee = useZap((s) => s.fee)
  const maxSpent = useZap((s) => s.maxSpent)
  const routes = useZap((s) => s.routes)
      ...
      <Collapse openDirection="upwards" className="w-full">
        <Collapse.Body>
          <Col className="gap-3 pb-3">
            <ZapCardItem fieldName="Addresses" tooltipContent={<ZapCardTooltipPanelAddress />} />
            <ZapCardItem
...
function ZapCardTooltipPanelAddress() {
  const coin1 = useZap((s) => s.coin1)
  const coin2 = useZap((s) => s.coin2)
  const routes = useZap((s) => s.routes)
  return (
    <div className="w-56">
      ...
      <Col className="gap-2">
        <ZapCardTooltipPanelAddressItem
          label={coin1?.symbol ?? '--'}
          type="token"
        ...
        {routes?.length === 1 ? (
          routes[0].source === 'serum' ? (
            ...
            <ZapCardTooltipPanelAddressItem label="Amm ID" type="ammId" address={String(routes[0].keys.id)} />
          ) : null
...
function ZapCardTooltipPanelAddressItem({
  className,
  label,
  address,
  type = 'account'
...
function KLineChart() {
  ...
  const [hasUISwrapped, { toggleZap: toggleUIZap, resetZapPosition }] = useZapTwoElements(kline1Box, kline2Box, {
    defaultHasWrapped: directionReversed
  })
