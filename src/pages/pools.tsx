...
import { AddLiquidityDialog } from '../components/dialogs/AddLiquidityDialog'
...
function PoolHeader() {
  ...
  return isMobile ? (
    ...
    <Grid className="grid-cols-[1fr,1fr] mobile:grid-cols-2 grid-flow-row-dense items-baseline gap-y-8 pb-8">
      <div className="text-2xl font-semibold text-white title mobile:text-lg justify-self-start">Pools</div>
      <Row className="items-center justify-self-end">
        <PoolStakedOnlyBlock />
      </Row>
    </Grid>
  )
...
function PoolStakedOnlyBlock() {
  ...
  return (
    <Row className="items-center justify-self-end mobile:justify-self-auto">
...
function PoolLabelBlock({ className }: { className?: string }) {
  return (
    <div className={className}>
      <div className="text-xl font-medium text-white mobile:text-base">Liquidity Pools</div>
...
function PoolCard() {
  ...
  const [isLiquidityShown, setIsLiquidityShown] = useState(false)
  ...
  const TableHeaderBlock = useCallback(
    () => (
      ...
      >
        ...
        <PoolRefreshCircleBlock className="self-center pr-8" />
      </Row>
    ),
  ...
  const innerPoolDatabaseWidgets = isMobile ? (
    <div>
      <Row className="gap-3 mb-4 mobile:mb-2 mobile:gap-2">
    ...
    <div>
      <Row className={'justify-between pb-5 gap-16 items-end'}>
        <PoolLabelBlock />
        <Row className="items-stretch gap-8">
          <PoolTimeBasisSelectorBox />
          <PoolSearchBlock />
        </Row>
      </Row>
  ...
  return (
    <CyberpunkStyleCard
      ...
      className="flex flex-col flex-grow w-full h-full p-10 pb-4 mobile:px-3 mobile:py-3"
      size="lg"
      style={{
        background:
          'linear-gradient(140.14deg, rgba(0, 182, 191, 0.15) 0%, rgba(27, 22, 89, 0.1) 86.61%), linear-gradient(321.82deg, #18134D 0%, #1B1659 100%)'
...
function PoolCardDatabaseBodyCollapseItemFace({
  ...
  const pcCotent = (
    ...
    >
      ...
      />
      <Grid className="mr-8 w-9 h-9 place-items-center">
...
function PoolCardDatabaseBodyCollapseItemContent({ poolInfo: info }: { poolInfo: HydratedPoolItemInfo }) {
  ...
  const isAddDialogOpen = useLiquidity((s) => s.isAddDialogOpen)
  ...
  return (
    ...
    >
      <CoinAvatarPair
        ...
        token2={info?.quote}
      />
      <Row className="flex-wrap items-center gap-2 font-medium mobile:text-xs mobile:mt-px">
        {info?.name}
