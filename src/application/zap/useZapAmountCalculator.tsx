...
export function useZapAmountCalculator() {
  ...
  /** for zap is always from up to down, up/down is easier to calc */
  ...
  useAsyncEffect(async () => {
    ...
    if (!upCoin || !downCoin || !connection || !pathname.startsWith('/zap')) {
      useZap.setState({
        fee: undefined,
    ...
    const focusDirectionSide = 'up' // temporary focus side is always up, due to zap route's `Trade.getBestAmountIn()` is not ready
...
}): Promise<ZapCalculatorInfo | undefined> {
